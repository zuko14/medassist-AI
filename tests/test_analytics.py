...
class TestTrackEvent:
    ...
    async def test_track_event_delegates(self, analytics_svc):
        with patch(
            "app.services.analytics.log_analytics_event", new_callable=AsyncMock
        ) as mock_log:
...
class TestDashboardStats:
    ...
    async def test_processes_appointment_data(self, analytics_svc):
        ...
        with patch("app.services.analytics.supabase") as mock_sb:
            ...
            mock_sb.table.return_value.select.return_value.gte.return_value.eq.return_value.execute.return_value = (
                mock_appts
            )
...
class TestRecentAppointments:
    ...
    async def test_returns_data(self, analytics_svc):
        ...
        with patch("app.services.analytics.supabase") as mock_sb:
            mock_sb.table.return_value.select.return_value.order.return_value.limit.return_value.eq.return_value.execute.return_value = (
                mock_result
            )
