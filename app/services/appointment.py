...
class AppointmentService:
    ...
    async def check_slot_availability(
        self, doctor_name: str, appointment_date: str, appointment_time: str
    ) -> bool:
    ...
    async def get_next_available_slots(
        self, doctor_name: str, from_date: Optional[str] = None, days: int = 7
    ) -> list[dict]:
        ...
        for i in range(days):
            ...
            if slots:
                available.append({"date": check_date, "slots": slots})

        return available

    async def find_alternative_doctors(
        self, department: str, exclude_doctor: str, from_date: Optional[str] = None
    ) -> list[dict]:
        ...
        for doc in doctors:
            ...
            for i in range(7):
                check_date = (
                    datetime.strptime(from_date, "%Y-%m-%d") + timedelta(days=i)
                ).strftime("%Y-%m-%d")
    ...
    async def get_appointment_summary(
        self, clinic_id: str, appointment_id: str
    ) -> Optional[dict]:
    ...
    async def get_upcoming_appointments(self, phone: str, days: int = 7) -> list[dict]:
    ...
    async def get_appointment_history(self, phone: str, limit: int = 10) -> list[dict]:
