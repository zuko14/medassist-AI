...
class PaymentService:
    ...
    async def create_booking_with_payment(
        ...
        hold_expires_at = (
            datetime.now(timezone.utc) + timedelta(minutes=settings.booking_hold_minutes)
        ).isoformat()
        ...
        try:
            ...
            if "duplicate" in error_msg or "unique" in error_msg or "violates" in error_msg:
                logger.info(f"Slot taken (DB constraint): {doctor_name} {appointment_date} {appointment_time}")
        ...
        try:
            ...
            # Log audit event
            self._log_payment_event(booking_id, "order_created", {
                "razorpay_order_id": razorpay_order_id,
                "amount_paise": amount_paise,
                "booking_ref": booking_ref,
            })
            ...
            try:
                ...
                self._log_payment_event(booking_id, "order_creation_failed", {
                    "error": str(e)[:500]
                })
            except Exception:
                pass
    ...
    async def process_payment_webhook(
        ...
        if not self.verify_webhook_signature(raw_body, signature, webhook_secret=webhook_secret):
            ...
            logger.warning("⚠️ Razorpay webhook SIGNATURE FAILED — possible spoofing attempt")
            await self._alert_admin("🚨 Payment webhook signature verification FAILED. Possible spoofing attempt.")
        ...
        existing_event = supabase.table("payment_events") \
            .select("id") \
        ...
        existing_confirmed = supabase.table("appointments") \
            .select("id") \
        ...
        if existing_confirmed.data:
            logger.info(f"Razorpay webhook: payment_id {payment_id} already processed (idempotent)")
        ...
        if not booking_result.data:
            ...
            self._log_payment_event_raw(None, "webhook_received", {
        ...
        self._log_payment_event(booking_id, "webhook_received", {
            "payment_id": payment_id,
            "order_id": order_id,
            "amount_paid": amount_paid,
        })

        self._log_payment_event(booking_id, "signature_verified", {
            "payment_id": payment_id,
        })
        ...
        if amount_paid != expected_amount:
            ...
            self._log_payment_event(booking_id, "mismatch_flagged", {
        ...
        if current_status not in ("pending_payment", "expired"):
            ...
            logger.warning(f"Booking {booking_id} in unexpected state '{current_status}' during webhook")
            self._log_payment_event(booking_id, "mismatch_flagged", {
        ...
        self._log_payment_event(booking_id, "confirmed", {
            "payment_id": payment_id,
    ...
    async def expire_stale_bookings(self) -> int:
        ...
        stale = supabase.table("appointments") \
            .select("*") \
            ...
            try:
                ...
                if order_id:
                    ...
                    if rz_status == "paid":
                        ...
                        self._log_payment_event(booking_id, "recovery_confirmed", {
                ...
                self._log_payment_event(booking_id, "hold_expired", {
    ...
    async def initiate_refund(self, booking_id: str, reason: str = "", clinic: Optional[dict] = None) -> dict:
        ...
        if slot_datetime:
            hours_until_slot = (slot_datetime - datetime.now(timezone.utc)).total_seconds() / 3600
        ...
        self._log_payment_event(booking_id, "refund_initiated", {
        ...
        try:
            ...
            self._log_payment_event(booking_id, "refund_completed", {
                "refund_id": refund_id,
                "amount_paise": booking["amount_paise"],
            ...
            self._log_payment_event(booking_id, "refund_failed", {
                "error": str(e)[:500],
                "payment_id": booking["payment_id"],
            })
    ...
    async def admin_confirm_booking(self, booking_id: str, admin_notes: str = "") -> dict:
        ...
        if booking["status"] != "pending_review":
            return {"success": False, "reason": f"can_only_confirm_pending_review_not_{booking['status']}"}

        supabase.table("appointments").update({
            "status": "confirmed"
        }).eq("id", booking_id).execute()

        self._log_payment_event(booking_id, "manual_confirm", {
    ...
    async def admin_reject_booking(self, booking_id: str, admin_notes: str = "") -> dict:
        ...
        if booking["status"] != "pending_review":
            return {"success": False, "reason": f"can_only_reject_pending_review_not_{booking['status']}"}
        ...
        self._log_payment_event(booking_id, "manual_reject", {
            "admin_notes": admin_notes,
        })
    ...
    async def get_daily_reconciliation(self, date_str: str = None) -> dict:
        ...
        pending_review = supabase.table("appointments") \
            .select("id") \
        ...
        return {
            ...
            "pending_review_count": len(pending_review.data) if pending_review.data else 0,
    ...
    def _log_payment_event(self, booking_id: str, event_type: str, payload: dict) -> None:
    ...
    def _log_payment_event_raw(self, booking_id: Optional[str], event_type: str, payload: dict) -> None:
        ...
        try:
            ...
            if booking_id:
                ...
                logger.warning(f"Payment event without booking_id: {event_type} — {json.dumps(payload, default=str)}")
                return
            supabase.table("payment_events").insert(data).execute()
        except Exception as e:
            logger.error(f"CRITICAL: Failed to write raw payment_event ({event_type}): {e}")
    ...
    async def _notify_payment_confirmed(self, booking: dict) -> None:
        ...
        try:
            ...
            try:
                ...
                date_display = dt.strptime(date_display, "%Y-%m-%d").strftime("%d %b %Y")
            except Exception:
                pass
            ...
            logger.info(f"Sent payment confirmation to {booking['patient_phone'][:6]}***")
