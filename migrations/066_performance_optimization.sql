...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_appointments_branch_id
    ON appointments(branch_id) WHERE branch_id IS NOT NULL;

-- appointments.lab_test_id → lab_tests(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_appointments_lab_test_id
    ON appointments(lab_test_id) WHERE lab_test_id IS NOT NULL;

-- payment_events.booking_id → appointments(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_events_booking_id
    ON payment_events(booking_id);

-- connector_audit_log.lab_report_id → lab_reports(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_connector_audit_lab_report
    ON connector_audit_log(lab_report_id) WHERE lab_report_id IS NOT NULL;

-- connector_audit_log.clinic_id → clinics(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_connector_audit_clinic_id
    ON connector_audit_log(clinic_id);

-- connector_failed_reports.clinic_id → clinics(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_connector_failed_reports_clinic
    ON connector_failed_reports(clinic_id);

-- admin_audit_logs.clinic_id → clinics(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_admin_audit_logs_clinic_id
    ON admin_audit_logs(clinic_id) WHERE clinic_id IS NOT NULL;

-- admin_audit_logs.user_id → clinic_admins(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_admin_audit_logs_user_id
    ON admin_audit_logs(user_id) WHERE user_id IS NOT NULL;

-- admin_notifications.admin_id → clinic_admins(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_admin_notif_admin_id
    ON admin_notifications(admin_id);

-- clinic_admins.clinic_id → clinics(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_admins_clinic_id
    ON clinic_admins(clinic_id) WHERE clinic_id IS NOT NULL;

-- lab_reports.matched_patient_id → patients(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lab_reports_matched_patient
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_rx_sends_prescription_id
    ON prescription_reminder_sends(prescription_id);

-- prescription_reminder_sends.clinic_id → clinics(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_rx_sends_clinic_id
    ON prescription_reminder_sends(clinic_id) WHERE clinic_id IS NOT NULL;

-- doctor_branch_assignments.doctor_id → doctors(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_doctor_branch_doctor_id
    ON doctor_branch_assignments(doctor_id);

-- doctor_branch_assignments.branch_id → branches(id)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_doctor_branch_branch_id
    ON doctor_branch_assignments(branch_id);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_admins_login
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_conversations_clinic_phone_v2
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_patients_clinic_phone_v2
