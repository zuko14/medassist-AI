...
| **"Multi-tenant PostgreSQL architecture with Row-Level Security (RLS)."** | RLS policies exist in `migrations/028`, `049`. **They are inert at runtime.** `app/database.py:36` builds the client from the `service_role` key, which holds `BYPASSRLS`; nothing connects as `kriya_app` or sets `app.clinic_id`, so the `service_role_all_*` policies (`USING (true)`) are what apply. `migrations/028` and `app/database.py:72` both say so in-repo, and `docs/architecture/tenant-isolation.md` documents it correctly. Isolation is enforced entirely in application code. | **CORRECTED 2026-08-30 — was wrongly marked VERIFIED** | *"Tenant data is isolated by enforced application-layer scoping on every query, backed by a database-level `chk_admin_scope` constraint and CI query-scoping linting."* **Do NOT claim RLS enforcement until T5.4 activates it.** |
