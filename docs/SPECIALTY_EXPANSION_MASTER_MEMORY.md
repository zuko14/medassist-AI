...
- **Intent Detection & Routing:**
  - When patient messages a specialty tenant:
    ...
    5. Patient describing a symptom (e.g. "severe toothache when biting" or "cloudy eyesight") triggers specialty symptom-to-treatment matching, showing the treatments the clinic offers that relate to it. **CORRECTED IN SESSION 10:** the bot never suggests "the exact clinical procedure". It names what the clinic treats and states that this is not a diagnosis; which procedure a patient needs is the examining doctor's call. Where the clinic has published a first-visit consultation (`care_pathway = 'entry'`, migration 081) the concern path offers that examination instead, and a doctor-decided procedure's card books an examination rather than the procedure.
...
| **Session 26** | 2026-09-26 | **Dental treatment plans** (migration 089, dental plan only): multi-sitting courses where each sitting is an appointment row, front-desk scheduling from free slots with any dentist, WhatsApp to patient (consent-gated) and doctor, post-sitting review buttons, owner-set atomic monthly message limits with add-on billing, Meta template submission per clinic. | `docs/sessions/SESSION_26_DENTAL_TREATMENT_PLANS.md` |
