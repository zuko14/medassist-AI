...
| `appointments` | Appointments | All | `booking` or `lab_test_booking` | `GET /admin/appointments`, `POST /admin/appointments/{id}/check-in`, `DELETE /admin/appointments/{id}` | Search box (patient name/phone, booking ref, doctor, test — all dates) plus date presets, basis toggle and status chips; data grid with phone under patient name, 12-hour times, status pills (`confirmed`, `checked_in`, `completed`, `cancelled`), cancel with reason dialog. |
